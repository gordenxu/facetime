...
def cli() -> None:
	...
	group_misc.add_argument('--log-level', help = wording.get('log_level_help'), default = 'debug', choices = logger.get_log_levels())
	# execution
	...
	group_face_selector.add_argument('--face-selector-mode', help = wording.get('face_selector_mode_help'), default = 'one', choices = facefusion.choices.face_selector_modes)
	...
	group_face_selector.add_argument('--reference-face-distance', help = wording.get('reference_face_distance_help'), type = float, default = 1.5, choices = facefusion.choices.reference_face_distance_range, metavar = create_metavar(facefusion.choices.reference_face_distance_range))
